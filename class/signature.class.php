...
class Signature{
    ...
    public function generateSignature($form,$secretKey){
        ...
        $this->db->query('INSERT INTO api_signature(sign,expire,create_time,form) VALUE(:sign,:expire,:create_time,:form)');
    ...
    public function create($sign_key,$type){
        $this->db->query('INSERT INTO api_signature(sign_key,expire,type) VALUE(:sign_key,:expire,:type)');
    ...
    public function get($sign){
        $this->db->query('SELECT sign FROM api_signature WHERE (sign = :sign AND expire > :now)');
        $this->db->bind(':sign'    ,$sign);
        ...
        if(!empty($data['sign']) && isset($data['sign'])){
            $this->db->query('UPDATE api_signature SET active_time = :active_time WHERE sign = :sign');
    ...
    private function removeSign($sign){
        $this->db->query('DELETE FROM api_signature WHERE sign = :sign');
        $this->db->bind(':sign',$sign);
        $this->db->execute();
    }

    private function clearSign(){
        $this->db->query('DELETE FROM api_signature WHERE (expire < :now)');
        $this->db->bind(':now',time());
        $this->db->execute();
    }
}
